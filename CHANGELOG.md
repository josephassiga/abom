...
- **Reference implementation** (`cli/src/abom/`): `bom.py` (build composition,
  append provenance, `abom-verify`), `audit.py` (tamper-evident hash chain),
  `cli.py`, `policy.py`.
- **Demo** (`cli/demo/`) — generate → verify → prove tamper-evidence end to end,
...
- The control-plane scaffold under `cli/src/abom/` (API, DB, orchestration) is a
